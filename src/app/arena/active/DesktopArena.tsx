...
import { 
  Activity, Sword, Shield, Zap, RefreshCw, Crown, 
  Sparkles, RotateCcw, X, ArrowLeftRight 
} from "lucide-react";
...
export default function DesktopArena({
  ...
  // State hover nội bộ cho Desktop
  ...
  // Lấy background từ store (nếu có) hoặc dùng mặc định
          ...
          <div className="relative z-20 w-full px-6 py-6 md:px-12 md:py-8 flex justify-between items-start">
            ...
            <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-500 w-72 shadow-lg backdrop-blur-md transition-opacity duration-300">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-xs text-slate-400 font-mono shrink-0 ml-2">Lv.{playerPokemon.level}</span>
              </div>
            ...
            <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-600 w-72 shadow-lg backdrop-blur-md transition-opacity duration-300">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-xs text-slate-400 font-mono shrink-0 ml-2">Lv.{enemyPokemon.level}</span>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-[240px] shrink-0 relative z-50">
          ...
          <div className="md:col-span-2 bg-slate-800 rounded-xl border border-slate-700 p-4 relative overflow-visible">
            ...
            {winner ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm z-30 animate-in fade-in rounded-xl">
                ...
                <button onClick={onReturnToLobby} className="bg-white text-black px-8 py-3 rounded-full font-bold hover:scale-105 transition-transform flex items-center gap-2 shadow-xl"><RotateCcw size={20} /> Return to Lobby</button>
              </div>
            ) : (mustSwitch || controlView === 'switch') ? (
              /* --- SWITCH POKEMON SCREEN --- */
              <div className="absolute inset-0 bg-slate-900 z-20 p-4 flex flex-col animate-in fade-in rounded-xl">
                <div className="flex justify-between items-center mb-4">
                  ...
                  {!mustSwitch && (<button onClick={() => setControlView('main')} className="text-slate-400 hover:text-white"><X size={24} /></button>)}
                </div>
                <div className="flex flex-wrap justify-center gap-3 w-full overflow-visible pb-2 px-4">
                  {myTeam.map((p, idx) => (
                    <button 
                      ...
                      onMouseEnter={() => setHoveredSwitchIdx(idx)} 
                      onMouseLeave={() => setHoveredSwitchIdx(null)} 
                      className={`
                        w-24 h-28 rounded-xl border-2 flex flex-col items-center justify-center gap-2 transition-all p-2 relative group 
                        ${idx === activePlayerIndex ? 'border-blue-500 bg-blue-900/20' : ''} 
                        ${p.currentHp === 0 ? 'opacity-40 border-slate-700 bg-slate-800 cursor-not-allowed grayscale' : 'border-slate-600 bg-slate-800 hover:bg-slate-700 hover:border-yellow-400 hover:-translate-y-1 cursor-pointer'} 
                        ${hoveredSwitchIdx === idx ? 'z-50' : 'z-0'}
                      `}
                    >
              ...
              /* --- BATTLE MOVES & ACTIONS --- */
              <div className="flex flex-col h-full gap-2">
                <div className="grid grid-cols-2 gap-3 flex-grow">
                  {playerPokemon.moves.map((move, idx) => (
                    // @ts-ignore
                    <button key={idx} onClick={() => handleAttack(move)} disabled={!isPlayerTurn} className="bg-slate-700 hover:bg-slate-600 border border-slate-600 hover:border-blue-400 rounded-lg px-3 py-2 disabled:opacity-50 transition-all group relative overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-2 rounded-lg flex items-center justify-center gap-2 transition-colors disabled:opacity-50 shadow-md ${pendingMechanic ? 'ring-2 ring-yellow-400 bg-indigo-700' : ''} `}
                  >
                    {pendingMechanic === 'mega' && <Crown size={18} className="text-yellow-400 animate-pulse" />}
                    {pendingMechanic === 'gmax' && <Zap size={18} className="text-red-400 animate-pulse" />}
                    {pendingMechanic === 'tera' && <Sparkles size={18} className="text-cyan-400 animate-pulse" />}

                    {!pendingMechanic && <Sparkles size={18} />}

                    <span>{pendingMechanic ? pendingMechanic.toUpperCase() : 'Metrics'}</span>
                  </button>
                  <button
            ...
            {!isPlayerTurn && !winner && !mustSwitch && (
              <div className="absolute inset-0 bg-slate-900/60 z-10 flex items-center justify-center backdrop-blur-[2px] cursor-not-allowed animate-in fade-in duration-300 rounded-xl">
