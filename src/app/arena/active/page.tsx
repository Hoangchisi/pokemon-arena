...
export default function BattlePage() {
  ...
  // --- TOOLTIP COMPONENT (Z-Index cực cao để đè lên mọi thứ) ---
  const PokemonStatsTooltip = ({ pokemon, side, className = "" }: { pokemon: any, side?: 'left' | 'right', className?: string }) => {
    // Logic vị trí mặc định nếu không truyền className
    const positionClass = className || (side === 'left' ? 'left-full ml-4 top-1/2 -translate-y-1/2' : 'right-full mr-4 top-1/2 -translate-y-1/2');

    return (
        <div 
            className={`
                absolute z-[100] bg-slate-900/95 border border-slate-500 p-3 rounded-xl shadow-2xl backdrop-blur-md w-48
                animate-in fade-in zoom-in-95 duration-200 pointer-events-none text-left
                ${positionClass}
            `}
        >
            <div className="border-b border-slate-700 pb-2 mb-2">
                ...
                <div className="flex gap-1 mt-1">
                    {pokemon.types.map((t: string) => (
                        <TypeBadge key={t} type={t} className="text-[9px] px-1.5 py-0.5" />
                    ))}
                </div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-2 overflow-hidden">
      ...
      <div className="max-w-6xl w-full h-[90vh] flex flex-col gap-4 scale-[0.9] origin-center">
        ...
        {/* === BATTLE SCREEN (LAYER THẤP z-10) === */}
        <div className="flex-grow bg-slate-900 rounded-2xl border border-slate-700 relative overflow-visible shadow-2xl flex flex-col z-10">
            ...
            <div className="bg-slate-800/90 p-4 rounded-xl border border-slate-500 w-72 shadow-lg backdrop-blur-sm transition-opacity duration-300">
              <div className="flex justify-between items-center mb-2">
                <div className="flex items-center gap-2 overflow-hidden min-w-0 flex-grow">
                    <span className="font-bold text-lg capitalize truncate text-white drop-shadow-md block">
                        {playerPokemon.name}
                    </span>
                    <div className="flex gap-1 shrink-0">
                        {playerPokemon.types.map((t: string) => (
                            <TypeBadge key={t} type={t} className="text-[9px] px-1.5 py-0.5 shadow-sm min-w-[32px] justify-center">
                                {t.slice(0, 3)}
                            </TypeBadge>
                        ))}
                    </div>
              ...
              {/* Thanh máu (Đã bao gồm số hiển thị bên trong) */}
              <HealthBar current={playerPokemon.currentHp} max={playerPokemon.maxHp} />
              
              {/* Chỉ hiển thị các pokeball còn lại, đã xóa dòng text HP bị trùng */}
            ...
            <div className="bg-slate-900/90 p-4 rounded-xl border border-slate-600 w-72 shadow-lg backdrop-blur-sm transition-opacity duration-300">
              <div className="flex justify-between items-center mb-2">
                <div className="flex items-center gap-2 overflow-hidden min-w-0 flex-grow">
                    ...
                    <div className="flex gap-1 shrink-0">
                        {enemyPokemon.types.map((t: string) => (
                            <TypeBadge key={t} type={t} className="text-[9px] px-1.5 py-0.5 shadow-sm min-w-[32px] justify-center">
                                {t.slice(0, 3)}
                            </TypeBadge>
                        ))}
          ...
          <div className="relative z-10 flex-grow w-full px-8 md:px-20 pb-8 flex justify-between items-end">
             ...
             >
                ...
                <img 
                  ...
                  className={`
                    w-48 h-48 md:w-64 md:h-64 pixelated object-contain drop-shadow-2xl transition-transform duration-300 ease-out 
             ...
             <div 
                className="relative group cursor-help"
                onMouseEnter={() => setHoveredSide('enemy')}
                onMouseLeave={() => setHoveredSide(null)}
             >
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-48 h-12 bg-black/60 blur-xl rounded-[100%]"></div>
                <img 
                  ...
                  className={`
                    w-40 h-40 md:w-56 md:h-56 pixelated object-contain drop-shadow-2xl transition-transform duration-300 ease-out
        ...
        {/* === CONTROLS (LAYER CAO z-50 ĐỂ TOOLTIP KHÔNG BỊ CHE) === */}
          ...
          <div className="md:col-span-2 bg-slate-800 rounded-xl border border-slate-700 p-4 relative overflow-visible">
            {winner ? (
             ...
             <div className="absolute inset-0 bg-slate-900 z-20 p-4 flex flex-col animate-in fade-in">
                ...
                {/* overflow-visible quan trọng để tooltip trồi ra ngoài */}
                  ...
                  {myTeam.map((p, idx) => (
                    <button
                      ...
                      className={`
                        ...
                        /* Tăng z-index khi hover để Tooltip con nằm trên các nút khác */
                    ...
                    >
                      ...
                      {/* Tooltip khi Hover Switch Menu */}
                      {hoveredSwitchIdx === idx && (
                          <PokemonStatsTooltip 
                             pokemon={p} 
                             className="bottom-full mb-2 left-1/2 -translate-x-1/2 w-48 z-[100] pointer-events-none"
                          />
                      )}
                    </button>
                  ))}
                ...
                <div className="grid grid-cols-2 gap-3 flex-grow">
                  {playerPokemon.moves.map((move, idx) => (
                    <button
                      ...
                      className="bg-slate-700 hover:bg-slate-600 border border-slate-600 hover:border-blue-400 rounded-lg px-3 py-2 disabled:opacity-50 transition-all group relative overflow-hidden"
                    >
